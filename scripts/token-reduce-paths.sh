...
if OUTPUT="$(TOKEN_REDUCE_DIAG_FILE="$DIAG_FILE" "$SCRIPT_DIR/token-reduce-search.sh" --paths-only "$QUERY")"; then
  ...
  # Note: STATUS must be captured as the first statement of this `else`.
  # An `if cmd; then ... fi` with no `else` resets $? to 0 after the `fi`
  # when the condition is false (POSIX: exit status is 0 when no branch's
  # condition tested true) -- so reading $? after `fi` instead of here would
  # always report exit_code=0, masking a genuine search failure as success.
  STATUS=$?
  END_MS="$(now_ms)"
  LATENCY_MS="$(( END_MS - START_MS ))"
  BACKEND="$(diag_value backend)"
