...
# Resolve uv absolute path at install time — hooks run with minimal PATH.
UV_ABS="$(command -v uv 2>/dev/null || echo uv)"
...
exec $UV_ABS run "$REPO_ROOT/scripts/token-reduce-update-check.py" --notify "\$@"
SHEOF
